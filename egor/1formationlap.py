...
keywords = ["Formula 1", "Formula1", "Formula.1", "Formula+1", "F1"]
...
# Function to check if a title matches the F1 keywords with word boundaries
# This prevents false positives like "SkyF1HD" matching the "F1" keyword
# Terms that should never match, even if a keyword is present
exclude_keywords = ["IndyCar", "Indy NXT", "NTT", "NASCAR", "ARCA"]

def title_matches(title):
    # First check exclusions
    title_lower = title.lower()
    if any(excl.lower() in title_lower for excl in exclude_keywords):
        return False
