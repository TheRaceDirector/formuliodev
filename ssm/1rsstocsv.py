...
with open('RssMagnets.txt', 'r') as file:
    rss_urls_encoded = file.read().splitlines()

# Decode the base64 encoded URLs
rss_urls = [base64.b64decode(url).decode('utf-8') for url in rss_urls_encoded]
...
for url in rss_urls:
    ...
    for entry in feed.entries:
        ...
        if any(keyword.lower() in entry.title.lower() for keyword in keywords) and year in entry.title:
            # Check if the title contains 'F1.Live' (case-insensitive)
            if "MULTi".lower() in entry.title.lower():
