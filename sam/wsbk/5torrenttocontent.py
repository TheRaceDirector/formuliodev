...
# Function to check if a file has a supported video extension
def is_supported_video_file(file_path):
    # Check if file extension is mp4, mkv, or avi (case insensitive)
    return bool(re.search(r'\.(mp4|mkv|avi)$', file_path, re.IGNORECASE))
        ...
        for csv_file in csv_files:
            ...
            with open(csv_file, 'r') as file, open(content_file_path, 'a', newline='') as content_file:
                ...
                for line in file:
                    ...
                    # Flag to track if at least one valid video file was found in this torrent
                    video_found = False

                    # Write the required information to content.csv for supported video files only
                    for file_index in range(num_files):
                        file_entry = file_storage.file_path(file_index)
                        if is_supported_video_file(file_entry):
                            video_found = True
                            content_writer.writerow([torrent_name, file_entry, infohash, file_index])
                            print(f'Added video file {file_entry} from torrent {torrent_name} with index {file_index} to content.csv')
                        else:
                            print(f'Skipping non-video file: {file_entry}')
                    
                    # If no valid video files were found, print a message
                    if not video_found:
                        print(f'No supported video files found in torrent: {torrent_name}')
